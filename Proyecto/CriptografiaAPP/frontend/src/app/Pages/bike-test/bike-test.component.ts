...
@Component({
  selector: 'app-bike-test',
  templateUrl: './bike-test.component.html',
  styleUrl: './bike-test.component.scss'
})
export class BikeTestComponent {
  ...
  async onAES(size): Promise<string[]> {
    return await this.getBikeRequest(await this.getBikeBody(size));
  }

  public async getBikeBody(size): Promise<FormData> {
    const formData = new FormData();
    formData.append("size", size);
    formData.append("algorithm", "BikeL3");
    return formData;
  }

  public async getBikeRequest(body: FormData): Promise<string[]> {
    let response;
