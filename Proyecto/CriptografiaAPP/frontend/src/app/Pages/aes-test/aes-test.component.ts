...
@Component({
  selector: 'app-aes-test',
  templateUrl: './aes-test.component.html',
  styleUrl: './aes-test.component.scss'
})
export class AESTestComponent {
  ...
  async onAES(size): Promise<string[]> {
    return await this.getAESRequest(await this.getAESBody(size));
  }

  public async getAESBody(size): Promise<FormData> {
    const formData = new FormData();
    formData.append("size", size);
    return formData;
  }

  public async getAESRequest(body: FormData): Promise<string[]> {
    let response;
    let res: string[] | undefined;
    try {
      response = await fetch("http://localhost:8080/criptografiaApp/webapi/CryptographyTest/AES", {
        method: "POST",
        body
      });
      if (response.ok) {
