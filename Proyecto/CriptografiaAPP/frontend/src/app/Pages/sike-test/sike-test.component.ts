...
@Component({
  selector: 'app-sike-test',
  templateUrl: './sike-test.component.html',
  styleUrl: './sike-test.component.scss'
})
export class SikeTestComponent {
  ...
  async onAES(size): Promise<string[]> {
    return await this.getSikeRequest(await this.getSikeBody(size));
  }

  public async getSikeBody(size): Promise<FormData> {
    const formData = new FormData();
    formData.append("size", size);
    formData.append("algorithm", "SikeP503");
    return formData;
  }

  public async getSikeRequest(body: FormData): Promise<string[]> {
    let response;
